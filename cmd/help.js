...
module.exports = {
    ...
    async run (client = new Client, message = new Message, args = Array) {
        ...
        // if (!(message.member.permissions.has('ADMINISTRATOR') || message.member.roles.cache.some(r=>sdata.role.includes(r.id)))) return message.channel.send(per).then(m => msgdelete(m, Number(process.env.deletetime)));
